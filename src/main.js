...
if (canvas) {
  ...
  const renderer = new THREE.WebGLRenderer({ canvas, antialias: !reduceMotion, alpha: true });
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, reduceMotion ? 1.5 : 2));
  ...
  controls.autoRotateSpeed = reduceMotion ? 0 : 0.6;
  ...
  coreMesh.rotation.set(0.5, 0.8, 0.2);
  wireMesh.rotation.set(-0.3, 0.4, 0.1);
  halo.rotation.z = 0.35;

  const particlesGeometry = new THREE.BufferGeometry();
  const particleCount = reduceMotion ? 320 : 880;
  ...
  const animate = () => {
    ...
    particlesMaterial.uniforms.uTime.value = reduceMotion ? 0 : elapsed;
